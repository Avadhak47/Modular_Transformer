...
def main():
    parser = argparse.ArgumentParser(description='Kaggle Robust Training Script')
    ...
    loader = MathematicalDatasetLoader(tokenizer_name=tokenizer_name, max_length=config.model.max_seq_len)
    ...
    if args.wandb:
        wandb.init(
            ...
            name=f"{config.training.experiment_name}{experiment_suffix}",
            config=config.training.__dict__,
            tags=['mathematical_reasoning', args.pe_type],
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        ckpt_filename = f"kaggle_epoch_{epoch}_{args.pe_type}_{timestamp}{experiment_suffix}.pt"
        ...
        if os.path.exists(input_base):
            for d in os.listdir(input_base):
                input_ckpt_dir = os.path.join(input_base, d, 'checkpoints')
                if os.path.exists(input_ckpt_dir):
                    remove_old_checkpoints(input_ckpt_dir, args.pe_type, ckpt_filename)
    ...
    if os.environ.get('KAGGLE_API') == '1':
        ...
        version_notes = f"Training completed - {args.pe_type} model, {args.epochs} epochs, experiment: {config.training.experiment_name}{experiment_suffix}"
